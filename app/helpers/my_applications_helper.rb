...
module MyApplicationsHelper
  ICON = {
    sent: { title: I18n.t('activerecord.attributes.my_application.statuses.sent'), class: 'bg-blue' },
    ...
    refused: { title: I18n.t('activerecord.attributes.my_application.statuses.refused'), class: 'bg-red' }
  }.freeze

  def status_label(enum_type)
    type = enum_type.to_s.to_sym
    tag.span ICON[type][:title], class: "label #{ICON[type][:class]}"
  end
end
